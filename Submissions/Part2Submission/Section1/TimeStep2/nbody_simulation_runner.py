...
class TimeStepEnergyTester:
    ...
    def run_test_series(self, 
        ...
        # Create plots of the results (both linear and logarithmic)
        ...
        # Create plot of time step vs final energy only
        self._create_final_energy_plot(dt_values, final_energy_values, output_dir)
    ...
    def _create_final_energy_plot(self, dt_values, final_energies, output_dir):
        """
        Create a focused plot showing only time step size vs final energy.
        ...
        try:
            ...
            # Plot final energy values with both linear and log scale variants
            # Linear plot
            plt.subplot(2, 1, 1)
            plt.plot(dt_values, final_energies, 'r-o', label='Final Energy')
            ...
            if np.any(nan_indices) and not np.all(nan_indices):
                ...
                if first_nan_idx > 0:
                    instability_dt = dt_values[first_nan_idx]
                    plt.axvline(x=instability_dt, color='r', linestyle='--', alpha=0.7)
                    plt.text(instability_dt, plt.ylim()[0], f'  Instability at dt={instability_dt:.4f}', 
                            verticalalignment='bottom', horizontalalignment='left')
            ...
            plt.grid(True)
            plt.title('Time Step vs Final Energy (Linear Scale)')
            plt.legend()
            ...
            plt.title('Time Step vs Final Energy (Logarithmic Scale)')
            plt.legend()
            
            # Save the plot
            plt.tight_layout()
            plt.savefig(f"{output_dir}/final_energy_vs_timestep.png", dpi=300)
            plt.close()
            
            self.logger.info(f"Final energy plot saved to {output_dir}/final_energy_vs_timestep.png")
            
        except Exception as e:
            self.logger.error(f"Error creating final energy plot: {str(e)}")
